...
"""Can be start with:
python -m DDP2.py
more config calls on: torchrun --help"""
...
def parse_args():
    ...
    parser.add_argument("--ngpus_per_node", default=3, type=int, help="number of GPUs per node for distributed training")
    parser.add_argument("--dist-url", default="tcp://127.0.0.1:12384", type=str, help="url used to set up distributed training")
...
def main(local_rank, ngpus_per_node, args):
    ...
    images_train, labels_train, vocab_train = dataProcess.images_labels(trans=trans, is_train=True, num_images=10000)
    train_data = dataProcess.GetDataset(images_train, labels_train)
    # test_data = datasets.FashionMNIST(root="data", train=False, download=True, transform=ToTensor())

    # initialize data loader
    # [*] using DistributedSampler
    batch_size = 32 * 3 // args.world_size  # [*] // world_size
    train_sampler = DistributedSampler(train_data, shuffle=True)  # [*]
    # test_sampler = DistributedSampler(test_data, shuffle=False)  # [*]
    train_dataloader = DataLoader(train_data, batch_size=batch_size, sampler=train_sampler)  # [*] sampler=...
    # test_dataloader = DataLoader(test_data, batch_size=batch_size, sampler=test_sampler)  # [*] sampler=...

    # initialize model
    model = resnet.resnet34().to(device)  # copy model from cpu to gpu
...
if __name__ == '__main__':
    ...
    os.system('export CUDA_VISIBLE_DEVICES=0,2,3')
    args = parse_args()
